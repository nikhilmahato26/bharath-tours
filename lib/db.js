...
export async function getAllPackagesAdmin() {
  ...
  await initDB()
  const { rows } = await pool.query(
    `SELECT p.id, p.data, p.category, p.status, p.agency_id, p.featured, p.featured_order, p.featured_at, p.featured_days,
...
export async function getFeaturedPackages() {
  ...
  await initDB()
  const { rows } = await pool.query(
    `SELECT id, data, category, status, agency_id, featured, featured_order, featured_at, featured_days
     FROM packages
